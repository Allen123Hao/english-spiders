...
class SingleWordSpider4JaBatch(scrapy.Spider):
    """单词爬虫 - 用于批量抓取指定单词的数据"""
    name = 'single_word_ja_batch'
    allowed_domains = ['dictionary.cambridge.org']
    
    def __init__(self, word_url=None, word=None, urls_file=None, *args, **kwargs):
...
if __name__ == '__main__':
    ...
    custom_settings = {
        'LOG_LEVEL': 'DEBUG',
        'LOG_FILE': 'single_word_spider_debug.log',
        'LOG_ENABLED': True,
        ...
        # 设置输出
        'FEEDS': {
            'output/cambridge_special_words_ja.json': {
                'format': 'json',
                'encoding': 'utf-8',
                'indent': 2,
                'overwrite': True,
    ...
    try:
        ...
        # 直接设置URL文件路径
        urls_file = "/Users/xyz/data/special-words-ja-url.log"  # 在这里直接设置URL文件路径
        ...
        # 批量模式
        print(f"启动批量爬虫，URL文件: {urls_file}")
        process.crawl(SingleWordSpider4JaBatch, urls_file=urls_file)
        process.start()
