...
class SingleWordSpider4JaParallel(scrapy.Spider):
    """单词爬虫 - 用于批量抓取指定单词的数据"""
    name = 'single_word_ja_batch_parallel'
    allowed_domains = ['dictionary.cambridge.org']
    
    def __init__(self, word_url=None, word=None, urls_file=None, start_index=0, end_index=None, *args, **kwargs):
    ...
    def _load_processed_urls(self):
        ...
        if os.path.exists(processed_file):
            try:
                ...
                self.logger.info(f"已加载 {len(self.processed_urls)} 个已处理的URL")
            except Exception as e:
                self.logger.error(f"加载已处理URL失败: {str(e)}")
    ...
    def _save_processed_urls(self):
        ...
        try:
            ...
            self.logger.error(f"保存已处理URL失败: {str(e)}")

    def start_requests(self):
        """开始请求"""
        if self.urls_file:
            try:
                # 使用生成器方式读取文件，避免一次性加载所有URL
                with open(self.urls_file, 'r', encoding='utf-8') as f:
                    urls = []
                    for i, line in enumerate(f):
                        if line.strip():
                            if i >= self.start_index and (self.end_index is None or i < self.end_index):
                                url = line.strip()
                                if url not in self.processed_urls:
                                    urls.append(url)
                
                self.total_urls = len(urls)
                self.logger.info(f"从文件读取到 {self.total_urls} 个URL")
                
                for url in urls:
                    self.current_url_index += 1
                    word = url.split('/')[-1]
                    self.logger.info(f"正在处理第 {self.current_url_index}/{self.total_urls} 个单词: {word}")
                    yield scrapy.Request(
                        url=url,
                        callback=self.parse_word_details,
                        errback=self.errback_httpbin,
                        meta={'letter': url.split('/')[-1][0].lower()},
                        dont_filter=True
                    )
            except Exception as e:
            ...
            word = self.word_url.split('/')[-1]
            self.logger.info(f"正在处理单词: {word}")
            yield scrapy.Request(
                url=self.word_url,
                callback=self.parse_word_details,
                errback=self.errback_httpbin,
                meta={'letter': self.word_url.split('/')[-1][0].lower()},
                dont_filter=True
            )
        else:
            self.logger.error("未提供URL或URL文件")
            return
...
if __name__ == '__main__':
    ...
    custom_settings = {
        ...
        'FEEDS': {
            'output/cambridge_special_words_ja_parallel.json': {
                'format': 'json',
                'encoding': 'utf-8',
                'indent': 2,
                'overwrite': True,
        ...
        'HTTPCACHE_EXPIRATION_SECS': 86400,  # 缓存过期时间（24小时）
        'HTTPCACHE_DIR': 'httpcache',  # 缓存目录
        'HTTPCACHE_IGNORE_HTTP_CODES': [503, 504, 522, 524],  # 忽略这些HTTP状态码的缓存
        'HTTPCACHE_STORAGE': 'scrapy.extensions.httpcache.FilesystemCacheStorage',  # 使用文件系统存储缓存
        'CLOSESPIDER_ITEMCOUNT': 1000000,  # 设置最大爬取数量
        'CLOSESPIDER_TIMEOUT': 86400,  # 设置最大运行时间（24小时）
        'CLOSESPIDER_ERRORCOUNT': 1000,  # 设置最大错误数
    }
    ...
    try:
        ...
        # 支持从命令行参数获取URL文件路径
        parser = argparse.ArgumentParser(description='剑桥词典爬虫')
        parser.add_argument('--urls-file', type=str, help='包含单词URL的文件路径')
        args = parser.parse_args()
        
        # 直接设置URL文件路径
        urls_file = "/Users/xyz/data/special-words-ja-url.log"
        
        if args.urls_file:
            urls_file = args.urls_file
            print(f"使用命令行参数中的URL文件: {urls_file}")
        else:
            print(f"使用默认URL文件: {urls_file}")
        
        # 创建爬虫进程
        process = CrawlerProcess(settings)
        
        # 启动爬虫
        print(f"启动爬虫，处理文件: {urls_file}")
        process.crawl(SingleWordSpider4JaParallel, urls_file=urls_file)
        process.start()
