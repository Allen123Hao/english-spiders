...
class SingleWordSpider(scrapy.Spider):
    """单词爬虫 - 用于抓取指定单词的数据"""
    name = 'single_word'
...
if __name__ == '__main__':
    ...
    try:
        ...
        # 启动爬虫（需要提供word_url参数）
        word_url = "https://dictionary.cambridge.org/dictionary/english-chinese-simplified/good"  # 示例URL
        word = word_url.split('/')[-1]  # 从URL中提取单词
        process.crawl(SingleWordSpider, word_url=word_url, word=word)
        process.start()
